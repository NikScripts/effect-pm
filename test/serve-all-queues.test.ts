  ...
  Effect.runPromise(
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* a.add({ n: 1 });
        yield* b.add([{ n: 2 }, { n: 3 }]);
        while ((yield* a.completed) < 1) yield* Effect.sleep(Duration.millis(10));
        while ((yield* b.completed) < 2) yield* Effect.sleep(Duration.millis(10));
        expect(yield* a.completed).toBe(1);
        expect(yield* b.completed).toBe(2);
      }).pipe(
