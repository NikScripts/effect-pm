...
// Regression: a `value` (stream-backed) field on a multiHost resource used to DEADLOCK the serve at build —
// `peersLayer` eagerly built each peer client, and a `value`'s client subscription blocked for the initial
// push, so a co-booting / down peer hung the whole host. Peer clients are now fully lazy: nothing connects
...
class SelfHost extends Resource.Host<SelfHost>("peers-lazy/Self") {}
class PeerHost extends Resource.Host<PeerHost>("peers-lazy/Peer") {}
class Fleet extends Resource.Tag<Fleet>()("peers-lazy/Fleet", {
  n: Resource.value(Schema.Number), // the trigger — a value field
}).pipe(Resource.multiHost([SelfHost, PeerHost])) {}
