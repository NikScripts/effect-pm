...
describe("SQLiteDurableQueueStore", () => {
  it.live("offer → take → complete preserves payload; sizes track pending", () =>
    Effect.gen(function* () {
      ...
      const taken = yield* store.take({ queueId: "q", leaseMillis: 1000 });
  ...
  it.live("take honors strict priority, then FIFO within a priority", () =>
    Effect.gen(function* () {
      const store = yield* makeStore;
      yield* store.offer({ id: "l", queueId: "q", priority: "low", payload: 0 });
      yield* store.offer({ id: "n1", queueId: "q", priority: "normal", payload: 0 });
      yield* store.offer({ id: "n2", queueId: "q", priority: "normal", payload: 0 });
      yield* store.offer({ id: "h", queueId: "q", priority: "high", payload: 0 });

      const order: Array<string> = [];
      for (let i = 0; i < 4; i++) {
        const t = yield* store.take({ queueId: "q", leaseMillis: 10_000 });
        if (Option.isSome(t)) {
  ...
  it.live("dedups on dedupKey, and escalates priority for a live key", () =>
    Effect.gen(function* () {
      const store = yield* makeStore;
      expect(
        yield* store.offer({ id: "1", queueId: "q", dedupKey: "k", priority: "low", payload: 1 }),
      ).toBe("inserted");
      expect(
        yield* store.offer({ id: "2", queueId: "q", dedupKey: "k", priority: "low", payload: 2 }),
      ).toBe("deduplicated");
      expect(yield* store.sizes("q")).toEqual({ high: 0, normal: 0, low: 1 });

      expect(
        yield* store.offer({ id: "3", queueId: "q", dedupKey: "k", priority: "high", payload: 3 }),
      ).toBe("escalated");
      const t = yield* store.take({ queueId: "q", leaseMillis: 1000 });
  ...
  it.live("a lease hides in-flight work; recover reclaims it (at-least-once)", () =>
    Effect.gen(function* () {
      const store = yield* makeStore;
      yield* store.offer({ id: "a", queueId: "q", priority: "normal", payload: 1 });

      const first = yield* store.take({ queueId: "q", leaseMillis: 60_000 });
      expect(Option.isSome(first)).toBe(true);
      // still leased → nothing else available
      expect(
        Option.isNone(yield* store.take({ queueId: "q", leaseMillis: 1000 })),
      ).toBe(true);

      expect(yield* store.recover("q")).toBe(1); // clears the lease
      const again = yield* store.take({ queueId: "q", leaseMillis: 1000 });
  ...
  it.live("fail requeues until maxAttempts, then dead-letters", () =>
    Effect.gen(function* () {
      ...
      yield* store.take({ queueId: "q", leaseMillis: 1000 }); // attempts = 1
      expect(yield* store.fail("a", { maxAttempts: 2 })).toBe("requeued");
      yield* store.take({ queueId: "q", leaseMillis: 1000 }); // attempts = 2
