...
// captureLogs + a HistoryStore, its logged line is captured, persisted, and read via logs.query.
class LogProc extends Process.Tag<LogProc>()(
  "test/process-log-history/Proc",
).pipe(Process.schedule([])) {}

it("process logs.query reads back captured logs (captureLogs + HistoryStore)", () =>
  Effect.runPromise(
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        while ((yield* proc.logs.query({})).length === 0) {
          yield* Effect.sleep(Duration.millis(10));
        }
      }).pipe(Effect.timeout(Duration.seconds(2)));

      const rows = yield* proc.logs.query({ limit: 50 });
...
it("process logs.query is empty without a HistoryStore (graceful, opt-in)", () =>
  Effect.runPromise(
    Effect.gen(function* () {
      const proc = yield* LogProc;
      yield* proc.run;
      yield* Effect.sleep(Duration.millis(50));
      expect(yield* proc.logs.query({})).toEqual([]);
    }).pipe(
      Effect.provide(
        Process.layer(LogProc, {
          effect: Effect.logInfo("process tick"),
