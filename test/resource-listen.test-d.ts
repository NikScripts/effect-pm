...
class Worker extends Resource.Node<Worker, Jobs | Emails>("listen-d/Worker", {
...
const full: Layer.Layer<any, any, any> = Resource.listen(Worker, [
  Resource.serve(Jobs, jobsImpl),
  Resource.serve(Emails, emailsImpl),
]);

// @ts-expect-error C3: Emails missing from listen catalog
const partial: Layer.Layer<any, any, any> = Resource.listen(Worker, [
  Resource.serve(Jobs, jobsImpl),
]);
...
const clients = Resource.clientsFor(Worker, Jobs, Emails);
void clients;

// @ts-expect-error C3: clientsFor must cover Emails
const clientsPartial = Resource.clientsFor(Worker, Jobs);
void clientsPartial;
