/**
 * @module examples/forms/hyperlink/node-nameless-listen-demo
 *
...
const program = Effect.gen(function* () {
  ...
  const serve = yield* spawner.spawn(
    ChildProcess.make(
      "pnpm",
      ["exec", "tsx", `${root}/examples/forms/hyperlink/node-nameless-listen-serve.ts`],
  ...
  const call = yield* spawner.spawn(
    ChildProcess.make(
      "pnpm",
      ["exec", "tsx", `${root}/examples/forms/hyperlink/node-nameless-listen-call.ts`],
      { cwd: root, stdout: "inherit", stderr: "inherit" },
    ),
  )
