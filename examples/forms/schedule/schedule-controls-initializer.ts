    ...
    schedule: ({ entries, set, add }) =>
      Effect.gen(function* () {
        ...
        if (existing.length === 1) {
          yield* add(
            ProcessSchedule.window("late-window", utcDateFromMillis(1_200), utcDateFromMillis(1_700)),
          );
        }
      }),
...
runNodeProgramOrExit(program, "form:schedule-controls-initializer finished");
