...
yield* proc.logs.stream                    // live captured lines
yield* proc.logs.query({ limit: 100 }) // past captured lines
```
...
// 1) backfill once
const recent = yield* queue.logs.query({ limit: 200 });
render(recent);

// 2) then tail live (no gap, no store access)
yield* queue.logs.stream.pipe(Stream.runForEach(render), Effect.forkScoped);

// live KPIs: read once via status.get, or follow status.changes / metrics.stream
...
`logs.query` / `metrics.query` are plain `query` verbs and cross RPC like `status.get`; `logs.stream` /
`metrics.stream` / `status.changes` are streams over the same transport. Runtime-wide logs use `HostLogs.byHost` /
...
|---|---|---|---|
| Queue | `logs.stream`, `metrics.stream` | `logs.query`, `metrics.query` | **done** (needs `captureLogs` for logs) |
| Process | `logs.stream` | `logs.query` | **done** (needs `captureLogs`) |
