...
| `src/ProcessStore.ts`, `src/ProcessStoreEvent.ts` | Layer combiner (transitional monolith) + shared event types. |
...
| `src/internal/store/spine.ts`, `composite.ts` | Shared storage plumbing — internal. |
