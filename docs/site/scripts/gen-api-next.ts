// Phase 3→5 BRIDGE (decision D4: cutover alongside + incremental). The gen-api writer, verbatim,
// with extraction swapped to the docgen services (TsProgram + Extractor.package). Writes to
// docs/site/api-data-next/ so the gate is a plain diff against the prototype's output:
//
//   npx tsx scripts/gen-api.ts && npx tsx scripts/gen-api-next.ts && diff -r api-data api-data-next
//
// Byte-identical output is the Phase 3 gate. Phase 5 renames this over gen-api.ts and deletes the
// prototype extractor; until then BOTH scripts stay, and any writer change must land in both.
//
//   tsx scripts/gen-api-next.ts [entryName ...]     (no args = all core entries)
...
// Split, per-page data lives under docs/site/api-data-next/ (one file per symbol + per-module
// summaries + a tiny index) — the -next dir keeps the gate a plain diff against api-data/.
const dataDir = nodePath.join(repoRoot, "docs/site/api-data-next");
