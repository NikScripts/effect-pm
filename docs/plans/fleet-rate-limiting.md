...
**Status:** Eng in progress — **R1 + R2 + R3 + R3b + R4 Eng’d** (adaptive 429 still opt-in deferred).  
...
| **R4** | ~~`Gate.HttpApiClient` Tag + nest absorb ApiMetrics~~ **Eng’d** — `httpApiClientLayer(Tag)`; `usage`/`windows` on nest; ApiMetrics deprecated. **Opt-in adaptive 429** still open |
