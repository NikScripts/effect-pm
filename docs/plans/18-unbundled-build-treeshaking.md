...
Make `import { QueueHyperlink } from "hyperlink-ts"` + `QueueHyperlink.Tag` (the **barrel
...
Public modules match Effect’s flat namespace layout (`src/QueueHyperlink.ts` ↔
...
- Subpath imports (e.g. `hyperlink-ts/QueueHyperlink`) are **proven engine-free** for Tag-only
  pulls. Prefer `import * as QueueHyperlink from "hyperlink-ts/QueueHyperlink"` in browser bundles.
...
The **barrel** `QueueHyperlink` is still easy for tsup/bundlers to materialize as a runtime object when
...
flattened). Then the consumer’s bundler resolves `QueueHyperlink.Tag` to the real module export and
drops the rest.
...
3. Verify with the bundle check below that the **barrel** `QueueHyperlink.Tag` excludes engine
