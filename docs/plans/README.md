...
- **Observe recipes (pipeable UI packs)** — universal `Observe.*`; family packs as values on a shared UI NS (working label `Bundle.*` — `Live` rejected); bind via `Observe.bind` / `Observe.use`. **Pack NS name open before Eng.** Plan: [observe-recipes.md](./observe-recipes.md).
