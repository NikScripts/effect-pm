...
- **Lifecycle kernel** — Effect-native Lifecycle (FiberHandle/Latch + Participating duals); tools via `start(jobs)` / `startFrom(Tag)` without kind switches. L0–L5 + A/C + B + P11 tip-synced. Plan: [lifecycle-kernel.md](./lifecycle-kernel.md) · locks: [lifecycle-kernel-decisions.md](../handoffs/lifecycle-kernel-decisions.md).
