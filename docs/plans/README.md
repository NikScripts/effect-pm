...
- **Observe recipes (pipeable UI packs)** — universal `Observe.*`; family packs as `*View.pack` on matching service view modules (`WorkPoolView.pack`, …); bind via `Observe.bind` / `Observe.use`. Retires `Bundle.observe`. Plan: [observe-recipes.md](./observe-recipes.md).
