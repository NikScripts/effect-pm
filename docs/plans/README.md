...
- **Observe recipes (pipeable UI packs)** — universal `Observe.atom` / `fn` / `scan` / `struct` / `merge`; family packs as values (`QueueObserve.live`); bind via `Observe.bind` / `Observe.use`. Retires `Bundle.observe` kind menu. Plan: [observe-recipes.md](./observe-recipes.md).
