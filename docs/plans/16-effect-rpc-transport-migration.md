...
`ControlTransportRpc` now provides the first Effect RPC transport adapter for
the existing `ControlProtocol` envelope. Existing HTTP transport remains
compatible and is still the default local/CLI path.
