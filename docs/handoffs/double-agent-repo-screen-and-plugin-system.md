...
**Unread** is a new local store (`sessionReads.ts`, AsyncStorage): a session is unread when `time.updated` > the timestamp it was last opened. `SessionChatScreen` marks read on open/leave; screens reload read-state on focus. (Currently surfaced on the repo screen; Home could adopt it too.)
