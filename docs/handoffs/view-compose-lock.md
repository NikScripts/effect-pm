...
HttpApi-shaped `Route` (`make` / `group` / `get` / `addHttpApi` / `match` / `urlBuilder`). Root endpoints + optional `topLevel`; mix Effect `HttpApi` via `addHttpApi`. See [`ui-routes-dream.md`](./ui-routes-dream.md). Navigator cutover next.
