...
**Status:** LOCKED (owner-directed), building.
**Thread:** follows the `Resource.http/ws/unix/nPipe` rename (client dialers match `Node.*` serve siblings).

## Two client families — the distinction is explicit

| Family | Bakes a transport? | Use |
|---|---|---|
| **`Resource.connect(tag, protocol)`** | **No** — you pass the `Layer<RpcClient.Protocol>`; connect imports zero transports | browser-safe; you choose+bundle exactly one wire |
| **`Resource.http/ws/unix/nPipe(node)`** | **Yes** — the wire is in the name, batteries included | server/CLI one-liners; a node already committed to a transport |
...
- `connect(Emails, Resource.protocolWebsocket("/rpc"))` — browser bundles only ws.
- `connect(Emails, Resource.protocolHttp(3009))` — server; replaces `clientHttp(Emails, 3009)`.
- Cast-free: `connect` is tag-only + generic (`<Self, S>`), so no `ResourceTag` TS2589.
...
```ts
const clientHost = Config.string("HOST").pipe(Config.nested("effectPmClient"), Config.withDefault("localhost"))
// reads EFFECT_PM_CLIENT_HOST via the ambient ConfigProvider (env by default)
```

- **Dev**: nothing set → `localhost`. Terse `protocolHttp(3009)` just works.
- **Prod**: app sets `EFFECT_PM_CLIENT_HOST=api.myapp.com` → every port shorthand becomes `http://api.myapp.com:3009/rpc`, automatically, everywhere.
...
2. `Resource.connect(tag, protocol)` (no-bake; sugar over `client(tag)` + the provided protocol).
