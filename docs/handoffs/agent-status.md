...
| **G / TUI** | `cursor/file-router-prototype-125f` | [typed JSX / compose](./view-compose-draft.md) · [page-layout](./page-layout-design.md) · [file-router](./file-router-prototype.md) | **Eng’d** — `last-ts/jsx-runtime` Element`<R>` + View`<P,R>`; Radix interop tests | work tip | last-ts tsc + view-jsx suites green | Page.Tag + tip-sync later | 2026-08-03 |
...
2. **Agent G / TUI:** last-ts P0–P3 + View handle/`fromEffect`/`gen` + **typed JSX erasure fix** (`last-ts/jsx-runtime`, Radix/shadcn interop tests) on work tip. **Need owner:** `Page.Tag` go, docs-site createPages cutover, tip-sync. P4 shims remain. Park: [`owned-string-casing-park.md`](./owned-string-casing-park.md).
