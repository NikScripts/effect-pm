...
| **G / TUI** | `cursor/file-router-prototype-125f` | [file-router](./file-router-prototype.md) · [Page naming](./view-page-naming.md) · [UI Route](./ui-routes-dream.md) | **Eng** — merged `integration`; Effect beta.102 + official `repos/effect`; `DashboardViews` + `componentsLayer` (no `DashboardLayer`); `Page.*` / fileRoot / vite / `hyp file-router`; Page.Tag deferred | work tip | typecheck green; file-router + lifecycle-defer-start tests | Page.Tag + docs-site createPages cutover | 2026-07-31 |
...
| **5** | `cursor/lifecycle-defer-start-929b` | [Lifecycle guide](../guides/lifecycle.md) · [kernel plan](../plans/lifecycle-kernel.md) · [P-locks](./lifecycle-kernel-decisions.md) · [launcher brief](./launcher-and-handoff-brief.md) | **tip-synced** — `stateRef`/`eventStream`/`asState`; wire-ready `impl`; Counter layer TLC | same tip as `integration` | typecheck green; lifecycle local+remote+contract 34/34 | Next: owner Gate stop-mode + P12; Agent G chrome | 2026-08-01 |
...
3. **Agent 5 (Launcher + handoff):** Lifecycle tip-synced (`stateRef` / `eventStream` / `asState`; wire-ready `impl`). P10 amended (Gate will participate — Eng blocked on stop-waiting mode). Next: owner Gate modes + P12; Agent G chrome. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
