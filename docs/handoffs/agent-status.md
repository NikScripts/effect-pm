...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. **Versioned + `Hyperlink.deprecated` Eng'd**. **Next:** Lookup-first launcher bring-up + **Lookup A/B** (#36 re-opened, high priority); then update-impact; then explicit A/B / `restartSuccessor`. Also gated: #37, Track D redirect. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
