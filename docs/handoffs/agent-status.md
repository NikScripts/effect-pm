...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/hyperservice-open-deps-5679` | [fleet rate limiting](../plans/fleet-rate-limiting.md) · [wire groups](../plans/wire-groups-and-identity.md) · [service shapes](../plans/service-shapes.md) | **Eng’d R4+adaptive** — awaiting tip-sync OK | work branch ahead of `integration` | typecheck 0 / api-metrics+http-api green | fleet-sync OK? | 2026-07-27 |
...
| **Docs site** | `cursor/docs-site-edge-cache-dbdc` (ε) | [origin down](./docs-site-origin-down.md) · edge cache · SEO/landing | **tip-synced** — SEO chrome + live landing CTAs + process-cache hot API JSON | same tip as `integration` | prior `/healthz` + CF HIT; redeploy in flight for SEO/landing | 1Password desktop CLI backup deferred (phone SSH) | 2026-07-27 |
...
1. **Agent 4:** W1–W3 + `default`/`defaults` + **R1–R3b tip-synced**; **R4 + adaptive 429 Eng’d** on work branch ([`../plans/fleet-rate-limiting.md`](../plans/fleet-rate-limiting.md)) — awaiting owner tip-sync OK. Incident: [`agent-04-w3-incident-2026-07-27.md`](./agent-04-w3-incident-2026-07-27.md).
...
4. **Docs site:** tip-synced on `integration` (`9cedc4884`). Live origin: OOM fix (2GB) + CF dep-API edge cache + dotenvx. On tip (needs redeploy): SEO chrome, landing CTAs, process-cache hot API JSON. 1Password CLI backup still deferred.
