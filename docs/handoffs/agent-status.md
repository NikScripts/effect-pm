...
| **5** | `cursor/lifecycle-defer-start-929b` | [Lifecycle guide](../guides/lifecycle.md) · [kernel plan](../plans/lifecycle-kernel.md) · [P-locks](./lifecycle-kernel-decisions.md) · [launcher brief](./launcher-and-handoff-brief.md) | **Eng** — Effect-shaped `make` + catchTag errors + events; Daemon on make; WorkPool still `of` | off `integration` | lifecycle tests 13/13 | L1 WorkPool `make` SSOT next; P1/P6/P9 still open; versioned / #35–37 deferred | 2026-07-31 |
...
3. **Agent 5 (Launcher + handoff):** Effect-shaped **Lifecycle.make** Eng’d (FiberHandle/Latch/events/`catchTag`); Daemon migrated; WorkPool still projects via `of`. Next: L1 WorkPool SSOT (P1/P6/P9). Branch `cursor/lifecycle-defer-start-929b`. Versioned / #35–37 deferred. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
