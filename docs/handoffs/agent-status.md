...
| **Docs site** | `cursor/docs-site-edge-cache-dbdc` (ε) | [dev host](./docs-site-dev-host.md) · edge cache · SEO | **tip-synced / live** — apex coming-soon (inlined CSS) / docs on `dev` | same tip as `integration` (`7a7e82bd3`) | smoke 38/38 on `dev`; CF Single Redirects; `/` has no external CSS | `@pm` alias; Getting Started polish; optional origin middleware for static | 2026-07-28 |
...
4. **Docs site:** Live — `hyperlink.cool` coming-soon (inlined CSS + `(book)` chrome split); docs at `https://dev.hyperlink.cool`. Tip `7a7e82bd3`, smoke 38/38, CF Single Redirects (origin middleware still bypassed by Waku static). Open: `@pm` alias, Getting Started polish.
