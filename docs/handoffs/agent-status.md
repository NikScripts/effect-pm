...
| **G / TUI** | `cursor/tui-dashboard-parity-125f` | TUI ↔ web Dashboard Group parity · **View compose** · F5 | **Eng** — `views` + WorkerPool `View.only` e2e | *(after push)* | ui-view 12 pass; example tsgo clean for card/app | Notes: [`view-tag-prototype.md`](./view-tag-prototype.md). Next: `ui.data` on compose; Page logs/schedule. Pre-existing: `ui/data` + `nodeConnect` tsgo noise. | 2026-07-28 |
