...
1. **Agent 4:** W1–W3 + `default`/`defaults` **on `integration`**. Next: **fleet rate limiting** proposal [`../plans/fleet-rate-limiting.md`](../plans/fleet-rate-limiting.md) (Effect `RateLimiter` + shared store; Gate substrate → HttpApiClient; ApiMetrics absorbed, not a migrate track). Waiting on owner locks (Redis vs HL store, Soft, nest name). Incident: [`agent-04-w3-incident-2026-07-27.md`](./agent-04-w3-incident-2026-07-27.md).
