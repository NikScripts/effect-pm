...
| **G / TUI** | `cursor/view-withsize-types-125f` | View compose · [UI Route](./ui-routes-dream.md) · [owned-string park](./owned-string-casing-park.md) | **tip-synced** — web/UI + owned-string Eng (ScheduleMode, reasons, UI modes) | same tip as `integration` | ui-router / daemon-schedule / widget-registry | park: value/ref, WorkPool modes, Href brand, Target.view | 2026-07-31 |
...
2. **Agent G / TUI:** web/UI refine + owned-string PascalCase Eng (clear tags/reasons); parked wire-heavy (`value`/`ref`, ScheduleMode, WorkPool modes) in [`owned-string-casing-park.md`](./owned-string-casing-park.md) — **tip-synced**.
