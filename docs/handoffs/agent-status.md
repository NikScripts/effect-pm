...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/beta0-release-prep-125f` | beta.0 polish + `Hyperlink.cli` TUI-default | **ready** — just `Hyperlink.cli` (+ TUI `serviceOption`); no bolted-on helpers | `f266e4e02` | typecheck+lint+resource-cli tests | Owner still: publish, docs domain; `hyp` = private repo-dev; public `hyperlink`/`hl` = app-wired `Hyperlink.cli`, not a shipped bin | 2026-07-24 |
