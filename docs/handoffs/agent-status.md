...
| **G / TUI** | `cursor/tui-dashboard-parity-125f` | TUI ↔ web Dashboard Group parity · `View` redesign | **Eng** — Dashboards on `DashboardViews.layer`; `forKind` base fallback-only | tip-sync after push | ui-dashboard-views + ui-view + workpool-view; tsgo ui/web/tui | Next: peel DetailScreen; **hold** kit Dashboard. Notes: [`client-adapters-design.md`](./client-adapters-design.md). | 2026-07-27 |
