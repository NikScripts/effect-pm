...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Versioned](./versioned-schema-decisions.md) · [deprecated](../guides/deprecated.md) · [Lifecycle](../guides/lifecycle.md) | **Already-up Eng'd** — fail default + adopt opt-in | same tip as `integration` | launcher-already-up + ensure-lookup | Next Eng: update-impact / restartSuccessor | 2026-08-05 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. **Lookup-first stack + app `alreadyUp` Policy Eng'd** (`NodeAlreadyUp` / adopt). **Next Eng:** update-impact / `restartSuccessor`. Also gated: #37, Track D redirect. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
