...
| **K** | `cursor/agent-k-page-route-6d0e` | [**corrections**](./last-ts-api-corrections.md) · [Document](./page-document-lock.md) · [Layout](./page-layout-lock.md) · [Page mint](./page-mint-lock.md) · [HttpApi](./router-httpapi-lock.md) | **ahead** — host prop adapt + Document + Layout + Page mint + Last site | `f113625e3` | last-ts tsc + host/page tests + docs/last/site tsc | Hyperlink `docs/site` still grandfathered | 2026-08-10 |
...
2. **Agent K:** corrections + host boundary + Document + Layout + Page mint + Last site Eng’d; `Server.fromPage(path, mint)` adapts Waku flats → soft-nav props. Park: [`owned-string-casing-park.md`](./owned-string-casing-park.md). Hyperlink `docs/site` cutover still later.
