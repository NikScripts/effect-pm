...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/beta0-release-prep-125f` | beta.0 polish + `Hyperlink.cli` TUI-default | **ready** — CLI+TUI control surface Eng’d (`cli(Group)`, bare→TUI via `Tui`/`serviceOption`) | sync after push | typecheck+lint+resource-cli tests | Owner still: publish, docs domain; `hyp` = private repo-dev; public `hyperlink`/`hl` = app-wired `Hyperlink.cli`, not a shipped bin | 2026-07-24 |
