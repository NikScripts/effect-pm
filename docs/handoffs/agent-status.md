...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/hyperservice-open-deps-5679` | [wire groups & identity](../plans/wire-groups-and-identity.md) · [service shapes](../plans/service-shapes.md) · [W3 incident](./agent-04-w3-incident-2026-07-27.md) | **W1–W3 Eng’d** — shared `Tag(wireKey, spec)` (no `*Family*`); ApiMetrics migrate + `.handle` rename **paused** | `efe9cab7` | typecheck + resource/shared-tag tests | **do not push `integration` without owner OK**; wait on metrics / handle name | 2026-07-27 |
...
1. **Agent 4:** Wire identity **W1–W3 Eng’d** ([`wire-groups-and-identity.md`](../plans/wire-groups-and-identity.md)) — shared Spec via `Hyperlink.Tag(wireKey, spec)` → `Factory<Self>()(instanceKey)`; ordinary `serve`/`client` (no `*Family*`). Demo: `examples/forms/resource/shared-tag-wire.ts`. **Paused:** ApiMetrics migration; Gate handle + reserved nest; `.handle` rename ([`service-shapes.md`](../plans/service-shapes.md)). Incident write-up [`agent-04-w3-incident-2026-07-27.md`](./agent-04-w3-incident-2026-07-27.md). **Never push `integration` without explicit owner authorization.**
2. **Agent G / TUI:** Dashboard Group parity + client-adapters design (`cursor/tui-dashboard-parity-125f`).
3. **Hygiene:** branch tidy **landed** on `integration` (`hyp` CLI + landing polish; Agent 1/3 tips retired).
