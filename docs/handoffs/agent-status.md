...
| **K** | `cursor/agent-k-page-route-6d0e` | [**context/view lock**](./last-context-view-lock.md) · [**spine**](./last-ts-spine.md) · [**last site**](../../docs/last/site/) · [**Catalog fromEffect**](../../examples/last/from-effect/Catalog.ts) · park: [**repo source viewer**](./repo-source-viewer-park.md) | **tip-synced** — `Router.link(catalog)`; Waku = location Layer only | same tip as `integration` | `example:last-router-context` + link test-d | `/repo/*` still parked | 2026-08-14 |
