...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **?** | `cursor/hyperservice-open-deps-5679` | [anyUnknown plan](../plans/any-unknown-in-error-context.md) — HS open `R` + docs | **batches 0–2 done** — tip-synced; awaiting owner call on residual errors / agent number | same tip as `integration` | serve/listen Dep type tests green; residual anyUnknown remain | Batches 3–5 open — may hand to another agent | 2026-07-24 |
| **4** | `cursor/beta0-release-prep-125f` | beta.0 polish + `Hyperlink.cli` TUI-default | **ready** — same tip as `integration`; diagnostic hacks removed | same tip as `integration` | typecheck: residual anyUnknown (intentional; no as-any-as hacks) | Owner still: publish, docs domain; `hyp` = private repo-dev; public `hyperlink`/`hl` = app-wired `Hyperlink.cli`, not a shipped bin | 2026-07-24 |
