...
| **5** | `cursor/lifecycle-defer-start-929b` | [addresses dock](./node-addresses-and-update-api.md) §3.3.3 | **design locked** — D1–D30; `.proxy()` on/off (no `"Prefer"`) | ahead of last tip-sync | owner: Ds good; make+add/extends like HttpApi | Eng when go (D20). Sync when Sync | 2026-08-16 |
