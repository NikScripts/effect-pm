...
| **G / TUI** | `cursor/file-router-prototype-125f` | [view compose / mount](./view-compose-draft.md) · [**upward provide**](./view-provide-draft.md) · [page-layout](./page-layout-design.md) · [file-router](./file-router-prototype.md) | **Eng’d down** + **provide spike** (`Last.provided`/`toLayer` → Context.Service Layer; tests green) | work tip | last-ts tsc + last-provide + view-jsx | View.gen Provides threading; Layout/Page; tip-sync later | 2026-08-04 |
...
2. **Agent G / TUI:** last-ts P0–P3 + bag compose/`View.mount` + **`Last.provided`/`toLayer` spike** (Context.Service value bags; incomplete ≠ Layer). Next: thread Provides through `View.gen`. **Need owner:** Layout/Page, `Page.Tag`, tip-sync. Park: [`owned-string-casing-park.md`](./owned-string-casing-park.md).
