...
| **K** | `cursor/agent-k-page-route-6d0e` | [**spine**](./last-ts-spine.md) · [**Catalog fromEffect**](../../examples/last/from-effect/Catalog.ts) · [**API audit**](./agent-invented-api-audit.md) | **tip-synced** — Catalog class + typed `fromEffect` / bake `R` | same tip as `integration` | example + `test/ui-from-effect*.ts` | audit deletes owner-gated | 2026-08-12 |
