...
| **K** | `cursor/agent-k-page-route-6d0e` | [**context/view lock**](./last-context-view-lock.md) · [**API audit**](./agent-invented-api-audit.md) · [**spine**](./last-ts-spine.md) | **on branch** — T2e context-link dogfood (no `provider(layer, Site)`) | ahead of tip | context-link example + Twoslash | tip-sync when green | 2026-08-15 |
