...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Update](../guides/update.md) · [addresses dock](./node-addresses-and-update-api.md) · [Versioned](./versioned-schema-decisions.md) | **design** — policy≠config; post-make config overlays | ahead of last tip-sync | tip: `pipe` + `withPolicy` already overlay; rename pending | Eng when go: `LookupConfig`/`NodeConfig` + `withConfig`; shrunk `LookupPolicy` (Yield/Pick). Dream rewrite; S14; stream/ref forward; Update parked; tip-sync when Sync | 2026-08-12 |
