...
| **Launcher** | `cursor/launcher-handoff-design-929b` | [launcher + handoff brief](./launcher-and-handoff-brief.md) | **bake** — product 1 + Track A API | branch tip | docs only | Effect-everywhere + `assume({ token })` locked. Open: timeout error tags, token format | 2026-07-27 |
...
3. **This agent (Launcher):** Effect-everywhere + `Launcher.Handle` / `assume({ token })` locked. Baking timeout errors before Eng. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
