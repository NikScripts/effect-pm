...
| **G / TUI** | `cursor/view-withsize-types-125f` | View compose · [UI Route](./ui-routes-dream.md) · [site routes](./waku-site-routes-api.md) | **soft-nav** — catalog `urls` + `Router.Link` (Waku; no layout Provider) | tip ahead of `integration` until sync | site typecheck | ask before sync | 2026-07-30 |
...
2. **Agent G / TUI:** Router dream — typed `urls` / catalog SSOT; site soft-nav via `Router.Link` (Waku) without layout Provider ([`waku-site-routes-api.md`](./waku-site-routes-api.md)). Ask before syncing `integration`.
