...
| **5** | `cursor/lifecycle-defer-start-929b` | [Lifecycle guide](../guides/lifecycle.md) · [kernel plan](../plans/lifecycle-kernel.md) · [P-locks](./lifecycle-kernel-decisions.md) · [launcher brief](./launcher-and-handoff-brief.md) | **tip-synced** — Gate Lifecycle polish (docs honesty, duals/readiness/remote, `RateLimiterError` value export) | same tip as `integration` | typecheck 0 `error TS`; gate-lifecycle + gate-remote-http + gate-pure | Next: Agent G chrome; launcher `#35–37` owner-gated | 2026-08-02 |
