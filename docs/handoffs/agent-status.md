...
| **5** | `cursor/lifecycle-defer-start-929b` | [Lifecycle](../guides/lifecycle.md) · [launcher + handoff brief](./launcher-and-handoff-brief.md) · [versioned schema](./versioned-schema-decisions.md) | **Eng** — Lifecycle building blocks (PascalCase, no kind helpers) + `deferStart` | off `integration` | lifecycle + work-pool tests | Versioned design-only (owner go); `restartSuccessor` / #35–37 deferred | 2026-07-31 |
...
3. **Agent 5 (Launcher + handoff):** Eng'ing **Lifecycle** + `Hyperlink.deferStart` on `cursor/lifecycle-defer-start-929b`. Versioned schema decisions still design-only. Next otherwise: `restartSuccessor` / #35–37. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
