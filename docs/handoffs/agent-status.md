...
| **Docs site** | `cursor/docs-site-edge-cache-dbdc` (ε) | [origin down](./docs-site-origin-down.md) · edge cache · SEO · Lighthouse | **tip-synced** — DurableWorkPoolStore + Getting Started refine | same tip as `integration` (`e8e488f02`) | typecheck + durable sqlite 10/10 | landing refresh proposals; redeploy; `@pm` alias | 2026-07-28 |
...
4. **Docs site:** tip-synced at `e8e488f02` (`DurableWorkPoolStore` rename + Getting Started refine). Landing refresh under owner review. Redeploy when ready. `@pm` alias still open.
