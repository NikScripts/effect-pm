...
| **3** | `cursor/store-read-rqb-where-906e` | [Logs store followers](./agent-03-logs-p1.md) · [plan](./agent-03-logs-store-followers-plan.md) | **`store-read-0` impl — CI local green** | tip | typecheck/test/lint green | Unified read payload + RQB `where` landed; await review / followers unlock | 2026-07-14 |
...
1. **Agent 3:** building **`store-read-0`** — remove per-shape read payload; baked-in payload + full nested RQB `where` ([plan](./agent-03-logs-store-followers-plan.md)).
...
### Ready / owner calls
4. After `store-read-0`: unlock store-follower slices `followers-0`…
