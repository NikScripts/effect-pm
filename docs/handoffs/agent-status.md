...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/tui-dashboard-parity-125f` | TUI ↔ web Dashboard Group parity | **ready for review** — Group Ink `Dashboard` + `Tui.open({ tree, path })`; local Fleet live-tested | `d04065803` | `vitest test/resource-cli`; `example:dashboard` / `example:hyperlink -- Mail` live vs servers | Sync to `integration` after owner OK; more leaf kinds (priority/gate/…) still web-ahead | 2026-07-24 |
