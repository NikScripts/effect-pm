...
2. **Agent G / TUI:** Router dream on `integration` through splat/`nodeHealth`/destinations check. Branch ahead: `catalog` SSOT (single path table → `Route.make` + derived Waku templates) ([`ui-routes-dream.md`](./ui-routes-dream.md)). Ask before sync.
