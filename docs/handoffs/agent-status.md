...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/hyperservice-open-deps-5679` | [fleet rate limiting](../plans/fleet-rate-limiting.md) · [wire groups](../plans/wire-groups-and-identity.md) · [service shapes](../plans/service-shapes.md) | **Eng** — R1+R3 fleet rateLimit (Gate+WorkPool presence store; shared-store tests); W1–W3 + `default`/`defaults` on `integration` | `665608a5` | typecheck + gate/fleet/work-pool tests | R2 observe nest · R4 HttpApiClient+ApiMetrics | 2026-07-27 |
...
1. **Agent 4:** W1–W3 + `default`/`defaults` **on `integration`**. **R1+R3 Eng’d** on work branch — Gate/WorkPool `rateLimit` + presence-driven store; shared-store fleet tests + recipe ([`../plans/fleet-rate-limiting.md`](../plans/fleet-rate-limiting.md)). Next: R2 observe nest → R4 HttpApiClient Tag + absorb ApiMetrics. Ask before landing on `integration`. Incident: [`agent-04-w3-incident-2026-07-27.md`](./agent-04-w3-incident-2026-07-27.md).
