...
| **5** | `cursor/lifecycle-defer-start-929b` | [Lifecycle guide](../guides/lifecycle.md) · [kernel plan](../plans/lifecycle-kernel.md) · [P-locks](./lifecycle-kernel-decisions.md) · [launcher brief](./launcher-and-handoff-brief.md) | **Eng** — WorkPool `stop` + `lifecycle._tag` SSOT; `autoStart` retired (`deferStart`) | `91a616bd` | typecheck + lifecycle/work-pool/handoff green | PR pending user create | 2026-07-31 |
...
3. **Agent 5 (Launcher + handoff):** L1 WorkPool SSOT Eng’d — `stop` verb, drop `status.phase`, retire `autoStart` (`Hyperlink.deferStart` only); badge via `lifecycle._tag`. Branch `cursor/lifecycle-defer-start-929b`. Versioned / #35–37 deferred. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
