...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Versioned](./versioned-schema-decisions.md) · [deprecated](../guides/deprecated.md) · [Lifecycle](../guides/lifecycle.md) | **Lookup-first Eng'd** — follow + handoff + `ensureLookup` | same tip as `integration` | ensure-lookup + follow suites | Next Eng: update-impact / app A/B | 2026-08-04 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. **Versioned + deprecated + `Lookup.follow` + ownership handoff + `Launcher.ensureLookup` Eng'd**. **Next Eng:** update-impact / app A/B. Also gated: #37, Track D redirect. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
