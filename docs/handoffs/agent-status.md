...
| **G / TUI** | `cursor/view-withsize-types-125f` | TUI ↔ web Dashboard Group parity · **View compose** · Tag mint | **tip-synced** — one-shot Tag mint; PropsOf / Service annotate | same tip as `integration` | typecheck 0; view suites 15/15 | Next: `ui.data` on compose; hold kit Dashboard | 2026-07-28 |
...
2. **Agent G / TUI:** View compose + one-shot Tag mint + `ViewKind` TaggedEnum on `cursor/view-withsize-types-125f` — lock [`view-compose-lock.md`](./view-compose-lock.md); guide [`../guides/view-tag-types.md`](../guides/view-tag-types.md). Next: `ui.data` on compose. Hold kit Dashboard.
