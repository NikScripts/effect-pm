...
| **G / TUI** | `cursor/file-router-prototype-125f` | [**page/route make lock**](./page-route-make-lock.md) · [last-ts site](./last-ts-site-framework.md) · [rsc-router](../last/rsc-router.md) | **Eng** `Page.make`/`static` + `docs/last/site` + `pageConfig` + `Route.fromPage` / `destinationsFromPages` — tip=`integration` | work tip | page-make + page-config-vite + route-from-page; `:5220` 200 | glob loader still open; owner: PR + `version` | 2026-08-08 |
