...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Update](../guides/update.md) · [addresses dock](./node-addresses-and-update-api.md) · [Versioned](./versioned-schema-decisions.md) | **design** — `Node.config` / `Node.policy` locked | ahead of last tip-sync | tip still `withPolicy` + Policy bags | Eng when go: `Node.config(node,{…})` + `Node.policy`; `LookupConfig`/`NodeConfig`; shrunk handler Policy. Dream rewrite; S14; stream/ref; Update parked; Sync | 2026-08-12 |
