...
| **5** | `cursor/lifecycle-defer-start-929b` | [addresses dock](./node-addresses-and-update-api.md) §3.3.3 | **design locked** — D1–D26; Node = HttpApi constructable | ahead of last tip-sync | owner: Ds good; easy/extendable bar; Node make/pipe/extends like HttpApi | Eng when go (D20): S14 → protocols+proxy auto dials → configure/policy → stream/ref → Update β → dream. Sync when Sync | 2026-08-15 |
