...
| **G / TUI** | `cursor/file-router-prototype-125f` | [view compose / mount](./view-compose-draft.md) · [page-layout](./page-layout-design.md) · [file-router](./file-router-prototype.md) | **Eng’d** — open-`R` `Unresolved` + bag `gen`/`succeed` + `View.mount`; JSX does not bubble `R` | work tip | last-ts tsc + view-jsx + Twoslash demo | Page.Tag + tip-sync later | 2026-08-03 |
...
2. **Agent G / TUI:** last-ts P0–P3 + View handle/`fromEffect`/`gen` + **bag compose + `View.mount`** (open-`R` not JSX-legal) on work tip. **Need owner:** `Page.Tag` go, docs-site createPages cutover, tip-sync. P4 shims remain. Park: [`owned-string-casing-park.md`](./owned-string-casing-park.md).
