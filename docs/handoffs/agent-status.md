...
| **G / TUI** | `cursor/view-withsize-types-125f` | View compose · [UI Route](./ui-routes-dream.md) · [site routes](./waku-site-routes-api.md) | **one service / two layers** — Waku adapts into shared `Service`; GroupNav on top | tip ahead of `integration` until sync | ui-router tests | ask before sync | 2026-07-31 |
...
2. **Agent G / TUI:** Router dream — one `Service`, lite + Waku layers (`ui/Router` / `ui/Router/waku`); GroupNav on top; site skins Waku ([`ui-routes-dream.md`](./ui-routes-dream.md)). Ask before syncing `integration`.
