...
| **K** | `cursor/agent-k-page-route-6d0e` | [**spine**](./last-ts-spine.md) · [**fromEffect demo**](../../examples/last/from-effect/) · [**API audit**](./agent-invented-api-audit.md) | **ahead** — NEW Context `fromEffect`/`groupsFromEffect` example + defer Eng | tip | example + `test/ui-from-effect.test.ts` | docs/site still on createPages; audit deletes owner-gated | 2026-08-11 |
