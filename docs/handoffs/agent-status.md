...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Update](../guides/update.md) · [addresses dock](./node-addresses-and-update-api.md) · [Versioned](./versioned-schema-decisions.md) | **reset** — owner rejected dream-redeploy β (2026-08-12) | ahead of last tip-sync | forward-proxy still useful as thin Shape β; dream recipe **not** SSOT | Rewrite dream: no dup workers / no manual queue move; use real WorkPool + Launcher handoff; easy compose API. Update parked; stream/ref forward; S14 peer rows; tip-sync when Sync | 2026-08-12 |
