...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Versioned](./versioned-schema-decisions.md) · [deprecated](../guides/deprecated.md) · [Lifecycle](../guides/lifecycle.md) | **deprecated Eng'd** — Lookup A/B design locked (single-addr + `follow`) | same tip as `integration` | deprecated + versioned tests | Next Eng: `Lookup.follow` + same-sock gap | 2026-08-03 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. **Versioned + `Hyperlink.deprecated` Eng'd**. **Lookup A/B design locked** (one address; orchestrator ownership handoff; `Lookup.follow` + Policy gap). **Next Eng:** `Lookup.follow` + Soft-bake same-sock replace; then orchestrated handoff / Lookup-first; then impact / app A/B. Also gated: #37, Track D redirect. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
