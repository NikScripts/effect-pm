...
| **5** | `cursor/lifecycle-defer-start-929b` | [Lifecycle guide](../guides/lifecycle.md) · [kernel plan](../plans/lifecycle-kernel.md) · [P-locks](./lifecycle-kernel-decisions.md) · [launcher brief](./launcher-and-handoff-brief.md) | **Eng** — tagged State/Event/`catchTag`; Daemon+WorkPool on `make` | tip | lifecycle + work-pool + handoff green | Drop `phase` dual / `shutdown`→`stop` / retire `autoStart` still open | 2026-07-31 |
