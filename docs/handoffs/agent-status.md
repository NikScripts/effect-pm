...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/hyperservice-open-deps-5679` | [service shapes plan](../plans/service-shapes.md) | **Eng’d** — tip-synced | same tip as `integration` | typecheck + promise suites | `Hyperlink.promise` shipped; next `pure` | 2026-07-26 |
...
1. **Agent 4:** Service shapes — Eng’d `Tag<Self, I>()`, `value`, overload arity, `Hyperlink.promise`. Next: `pure`. Creating a Hyperlink polish still paused.
