...
| **G / TUI** | `cursor/file-router-prototype-125f` | [**router HttpApi lock**](./router-httpapi-lock.md) · [effect app router](./effect-app-router-plan.md) | **Eng’d** `group.from(Service)` + resolve in `RouterBuilder.layer`; `Router.layerDestinations`; flat `Group.asRoutes` | work tip | router-builder + ui-router/routes green | real `Waku.layer`; retire `Route.handle` Outlet path | 2026-08-05 |
