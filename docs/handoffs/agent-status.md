...
| **Docs site** | `cursor/docs-site-edge-cache-dbdc` (ε) | [origin down](./docs-site-origin-down.md) · edge cache · SEO · Lighthouse | **shipping** — React #418 table thead/tbody fix; tip-synced | same tip as `integration` | vitest 68/68; redeploy + LH re-score in progress | 1Password CLI deferred; **no landing UI without owner OK** | 2026-07-27 |
...
4. **Docs site:** tip-synced. Fixing React **#418** on table pages (`thead`/`tbody` + skip empty Djot caption). Redeploy + install LH re-score next. Landing UI unchanged. 1Password CLI deferred.
