...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/hyperservice-open-deps-5679` | [wire groups & identity](../plans/wire-groups-and-identity.md) · [service shapes](../plans/service-shapes.md) · [W3 incident](./agent-04-w3-incident-2026-07-27.md) | **W1–W3 + `default`/`defaults` Eng’d** — `pure` retired; ApiMetrics migrate **paused** | *(update on push)* | typecheck + resource-default tests | **do not push `integration` without owner OK**; wait on metrics / `cell` | 2026-07-27 |
...
1. **Agent 4:** Wire identity **W1–W3 Eng’d** ([`wire-groups-and-identity.md`](../plans/wire-groups-and-identity.md)) — shared Spec via `Hyperlink.Tag(wireKey, spec)` → `Factory<Self>()(instanceKey)`. **`default`/`defaults` Eng’d** — `Hyperlink.pure` retired ([`service-shapes.md`](../plans/service-shapes.md)). Demo: `examples/forms/resource/shared-tag-wire.ts`. **Paused:** ApiMetrics migration; Gate reserved nest; live `cell`. Incident write-up [`agent-04-w3-incident-2026-07-27.md`](./agent-04-w3-incident-2026-07-27.md). **Never push `integration` without explicit owner authorization.**
