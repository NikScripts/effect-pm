...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Versioned](./versioned-schema-decisions.md) · [deprecated](../guides/deprecated.md) · [Lifecycle](../guides/lifecycle.md) | **tip-synced** — Dialers census + sticky dual-serve (`restartSuccessor` prefer) | same tip as `integration` | typecheck + plan-update / restartSuccessor / follow / peers | Explicit Redirect SDK deferred; stream resume optional | 2026-08-07 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. **Lookup-first + already-up + `Lookup.planUpdate` + `Launcher.restartSuccessor` + Dialers / sticky dual-serve Eng'd**. Optional: stream resume tokens / seam dedupe. Gated: #37. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
