...
| **5** | `cursor/lifecycle-defer-start-929b` | [Lifecycle guide](../guides/lifecycle.md) · [kernel plan](../plans/lifecycle-kernel.md) · [P-locks](./lifecycle-kernel-decisions.md) · [launcher brief](./launcher-and-handoff-brief.md) | **tip-synced** — L0–L7; engine `run` errors (`GateStopped` + `RateLimiterError`) on wire — no erase | same tip as `integration` | typecheck 0 `error TS`; gate-handle.test-d + gate suites | Next: Agent G chrome; launcher `#35–37` owner-gated | 2026-08-02 |
...
3. **Agent 5 (Launcher + handoff):** Lifecycle kernel **L0–L7 Eng’d** tip-synced; Gate wire `run` carries **`GateStopped` + `RateLimiterError`** (honest errors — no erase). P12 Rejected. Next: Agent G chrome; launcher `#35–37` / `restartSuccessor` deferred (no Eng until API locked). Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
