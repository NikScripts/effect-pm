...
| **G / TUI** | `cursor/file-router-prototype-125f` | [file-router](./file-router-prototype.md) · [Page naming](./view-page-naming.md) · [UI Route](./ui-routes-dream.md) | **tip-synced** — Dashboard thin shells; one `ui/DashboardViews`; Effect beta.102; `Page.*` / file-router | same tip as `integration` | typecheck; ui-dashboard-views + ui-file-router + gate-lifecycle | Next: Page.Tag + docs-site createPages cutover | 2026-08-02 |
...
| **5** | `cursor/lifecycle-defer-start-929b` | [Lifecycle guide](../guides/lifecycle.md) · [kernel plan](../plans/lifecycle-kernel.md) · [P-locks](./lifecycle-kernel-decisions.md) · [launcher brief](./launcher-and-handoff-brief.md) | **tip-synced** — L0–L7 Eng’d; standards hygiene (no `@since`; guide/plan SSOT; cast-free `Lifecycle.make`) | same tip as `integration` | typecheck 0 `error TS`; gate+lifecycle suites | Next: Agent G chrome; launcher `#35–37` owner-gated; `RateLimiterError` wire optional | 2026-08-02 |
...
3. **Agent 5 (Launcher + handoff):** Lifecycle kernel **L0–L7 Eng’d** tip-synced (P10 Gate + `GateStopped` on wire; P12 Rejected). Standards hygiene landed. Next: Agent G chrome; launcher `#35–37` / `restartSuccessor` deferred (no Eng until API locked). Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
