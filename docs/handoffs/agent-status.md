...
| **5** | `cursor/lifecycle-defer-start-929b` | [Lifecycle guide](../guides/lifecycle.md) · [kernel plan](../plans/lifecycle-kernel.md) · [P-locks](./lifecycle-kernel-decisions.md) · [launcher brief](./launcher-and-handoff-brief.md) | **Gate Lifecycle Eng’d** — P10 shipped (`GateStopped` + `stopMode`; pause hold-all; live `setConcurrency`/`setRateLimit`) | on work branch (pending tip-sync) | typecheck 0 errors; `gate` + `gate-lifecycle` + `lifecycle-defer-start` 57/57 | Next: owner P12; Agent G chrome | 2026-08-01 |
