...
| **3** | `cursor/remove-persist-layer-906e` | [Logs store followers](./agent-03-logs-p1.md) · [plan](./agent-03-logs-store-followers-plan.md) | **persistLayer/LogStore removed** | pending land | typecheck / 444 tests | Hard-delete interim sink; store-layer memo deferred | 2026-07-14 |
...
1. **Agent 3:** hard-removing interim `Logs.persistLayer` / `LogStore` on `cursor/remove-persist-layer-906e` (followers already on `integration`; store-layer `lineId` memo deferred).
