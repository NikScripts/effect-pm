...
| **K** | `cursor/agent-k-page-route-6d0e` | [**context/view lock**](./last-context-view-lock.md) · [**spine**](./last-ts-spine.md) · [**last site**](../../docs/last/site/) · [**Catalog effect**](../../examples/last/from-effect/Catalog.ts) · park: [**repo source viewer**](./repo-source-viewer-park.md) | **in flight** — outdated API cleanup Eng’d (`View`→`ViewFn`; stamp/RouterClient gone) | `4afd57fbd`+ | last-ts typecheck + router/document/view vitest green | tip-sync; stale handoff docs still mention stamp/RouterClient | 2026-08-15 |
