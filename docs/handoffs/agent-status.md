...
| **Docs site** | `cursor/docs-site-edge-cache-dbdc` (ε) | [origin down](./docs-site-origin-down.md) · edge cache · SEO · Lighthouse | **Eng** — rename landed + Getting Started refine | `221c80fa0`+ | DurableWorkPoolStore + intro→`Node.http`; Creating/Install polish | tip-sync + redeploy OK?; owner style pass; `@pm` alias | 2026-07-28 |
...
4. **Docs site:** `DurableQueueStore` → `DurableWorkPoolStore` Eng'd (`221c80fa0`). Getting Started refine in flight (Intro uses day-1 `Node.http`; Creating checklist; Install editor trim). Needs tip-sync OK + redeploy. `@pm` alias still open.
