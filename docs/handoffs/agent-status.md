...
| **G / TUI** | `cursor/view-withsize-types-125f` | View compose · [UI Route](./ui-routes-dream.md) | **tip-synced** — `Route` + `groupRoute.routes`; Navigator cutover next | same tip as `integration` | ui-routes 5/5 | F5 flake pre-existing | 2026-07-29 |
=======
...
| **5** | `cursor/launcher-handoff-design-929b` | [launcher + handoff brief](./launcher-and-handoff-brief.md) · [rename](./rename-hyperlink-handoff.md) | **Eng'd** — serve-site `{ handoff }` fn (#39); `withHandoff` retired | merging `integration` | typecheck 0 + handoff tests 13/13 | serve `{ node }` re-stamp + `restartSuccessor` deferred; #35–37 deferred | 2026-07-29 |
...
=======
2. **Agent G / TUI:** Dashboard unheld; UI `Route` + `groupRoute.routes` Eng’d ([`ui-routes-dream.md`](./ui-routes-dream.md)); Navigator cutover next. Branch `cursor/view-withsize-types-125f`.
=======
