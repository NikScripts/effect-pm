...
### Ready / owner calls
4. **Node catalog + identity lookup** — design [`node-catalog-and-discovery.md`](./node-catalog-and-discovery.md); Phase-2 **C1–C5** + Phase-3 directory + **D7 vertical** Eng on tip; D3 bare `distributed` / peersLayer directory + dynamic `instance` still **LEAN**; D4 picker / managers / X1 still OPEN
