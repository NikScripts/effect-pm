...
| **G / TUI** | `cursor/file-router-prototype-125f` | [**router HttpApi lock**](./router-httpapi-lock.md) · [last-ts site](./last-ts-site-framework.md) · [effect app router](./effect-app-router-plan.md) | **Eng** site phases 1–4: View defaults, book chrome, `Router.make` catalog, RouterBuilder/Page demo | work tip | view-service-default + book-chrome + router-page-demo + last-ts tsc | Waku `Page.*` cutover; hyperlink migrate; owner: PR + `version` | 2026-08-07 |
...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Update + addresses](./node-addresses-and-update-api.md) · [Versioned](./versioned-schema-decisions.md) · [Lifecycle](../guides/lifecycle.md) | **tip-synced** — sync-first: `Node.Tag` never became `Service`; `Node.make` dock + G | same tip as `integration` | design doc only (no Eng) | **G: read §7.5–7.6** make vs Tag/Service; Host/Machine still open | 2026-08-08 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. Lookup-first + planUpdate + Dialers / sticky dual-serve Eng'd. **`restartSuccessor` rejected as SSOT** — design dock [`node-addresses-and-update-api.md`](./node-addresses-and-update-api.md). **Sync (5 first):** `Node.Tag` was never renamed to `Node.Service` (still Tag; is Context.Service under the hood). Lean `Node.make` (HttpApi-like) + locality under Node; keep Launcher. **Agent G:** read dock §7.5–7.6 before Page.Tag vs make. Gated: #37.
