...
| **G / TUI** | `cursor/view-withsize-types-125f` | TUI ↔ web Dashboard Group parity · **View compose** · thin-handle helpers | **in progress** — standards: handles stay thin | ahead of `integration` (standards) | typecheck n/a (docs) | Prefer `observe(tag)` helper over `ui.data.*` / methods on Tag; Hold kit Dashboard | 2026-07-28 |
...
2. **Agent G / TUI:** View compose + Tag mint on `cursor/view-withsize-types-125f` — lock [`view-compose-lock.md`](./view-compose-lock.md); principle [`../standards/principles.md#handles-stay-thin`](../standards/principles.md#handles-stay-thin) (thin handles; helpers, not `ui.data.*` / Tag methods). Hold kit Dashboard.
