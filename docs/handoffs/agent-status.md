...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Versioned](./versioned-schema-decisions.md) · [deprecated](../guides/deprecated.md) · [Lifecycle](../guides/lifecycle.md) | **`Lookup.follow` Eng'd** — same-sock gap Policy | same tip as `integration` | lookup-follow + related dial suites | Next Eng: orchestrated Lookup handoff → ensure-Lookup-first | 2026-08-03 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. **Versioned + `Hyperlink.deprecated` + `Lookup.follow` Eng'd**. **Next Eng:** orchestrated Lookup ownership handoff (same address); then Launcher ensure-Lookup-first; then impact / app A/B. Also gated: #37, Track D redirect. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
