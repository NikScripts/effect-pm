...
| **5** | `cursor/lifecycle-defer-start-929b` | [addresses dock](./node-addresses-and-update-api.md) §3.3.3 | **design locked** — D1–D22 + easy/extendable bar | ahead of last tip-sync | owner: Ds good; quality bar = best-in-class A/B DX | Eng when go (D20): S14 → configure/policy → stream/ref → Update β → dream. Tip-sync when Sync | 2026-08-15 |
