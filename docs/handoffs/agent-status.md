...
| **G / TUI** | `cursor/file-router-prototype-125f` | [**effect app router plan**](./effect-app-router-plan.md) · [provide](./view-provide-draft.md) · [compose](./view-compose-draft.md) · [file-router](./file-router-prototype.md) | **Effect-native Views** — `Layer.succeed`/`effect` + `Effect.gen`; `View.mount(Service)` only; no View Layer masks | work tip | full tsc + view/last-provide tests | B1 body-only Page; tip-sync later | 2026-08-04 |
