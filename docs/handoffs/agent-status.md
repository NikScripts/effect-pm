...
| **G / TUI** | `cursor/view-withsize-types-125f` | View compose · [UI Route](./ui-routes-dream.md) · [owned-string park](./owned-string-casing-park.md) | **tip-synced** — `_tag` cutover + routing docs/examples/tests | same tip as `integration` | ui-router 18/18 + group-nav example | park: value/ref, WorkPool modes, Href brand | 2026-07-31 |
...
| **5** | `cursor/lifecycle-defer-start-929b` | [Lifecycle guide](../guides/lifecycle.md) · [kernel plan](../plans/lifecycle-kernel.md) · [P-locks](./lifecycle-kernel-decisions.md) · [launcher brief](./launcher-and-handoff-brief.md) | **tip-synced** — P10 Gate Lifecycle audit-fixed (cast, pause-holds-waiting test, docs honesty); P12 Rejected | same tip as `integration` | typecheck 0 `error TS`; gate+lifecycle suites green (gate-lifecycle 7/7) | Residual: Tag `run` erases `GateStopped` (same as `RateLimiterError`); Agent G chrome; launcher deferred | 2026-08-01 |
...
3. **Agent 5 (Launcher + handoff):** Lifecycle + **P10 Gate Eng’d** tip-synced; critical audit fixed (no `input as T`; pause-holds-waiting coverage; guide/TSDoc honest about wire `GateStopped` erasure). **P12 Rejected** — nothing `@locked` anywhere. Next: Agent G chrome; launcher `#35–37` / `restartSuccessor` deferred. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
