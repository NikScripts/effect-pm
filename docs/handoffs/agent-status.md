...
| **G / TUI** | `cursor/file-router-prototype-125f` | [**router HttpApi lock**](./router-httpapi-lock.md) · [effect app router](./effect-app-router-plan.md) | **Eng’d** `Route.Page` + `Route.get`→HttpApiEndpoint; Handlers dispatch Page/Json; UI match Page-only | work tip | ui-routes + router-builder (+ mixed) green; last-ts tsc green | thin-wrap catalog onto Effect `HttpApi`; wire Api handlers to HttpRouter | 2026-08-06 |
