...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/hyperservice-open-deps-5679` | [wire groups & identity](../plans/wire-groups-and-identity.md) · [service shapes](../plans/service-shapes.md) · [W3 incident](./agent-04-w3-incident-2026-07-27.md) | **synced to `integration`** — W1–W3 + `default`/`defaults` (`pure` retired) | `19c538b2` | typecheck + resource-default tests | next unlock: ApiMetrics / Gate nest / `cell` | 2026-07-27 |
...
| **Launcher** | `cursor/launcher-handoff-design-929b` | [launcher + handoff brief](./launcher-and-handoff-brief.md) · named-handles doc refresh | **plan-first** + docs hygiene | branch tip | docs only | Launcher bake parked; refreshed Agent D naming (`WorkPool`/`Gate`, old names purged from living handoffs) | 2026-07-27 |
...
1. **Agent 4:** Wire identity **W1–W3 Eng’d** ([`wire-groups-and-identity.md`](../plans/wire-groups-and-identity.md)) — shared Spec via `Hyperlink.Tag(wireKey, spec)` → `Factory<Self>()(instanceKey)`. **`default`/`defaults` Eng’d** — `Hyperlink.pure` retired ([`service-shapes.md`](../plans/service-shapes.md)). Demos: `examples/forms/resource/shared-tag-wire.ts`, `default-defaults.ts`. **Paused:** ApiMetrics migration; Gate reserved nest; live `cell`. Incident: [`agent-04-w3-incident-2026-07-27.md`](./agent-04-w3-incident-2026-07-27.md). Owner authorized sync of this slice to `integration`.
