...
| **G / TUI** | `cursor/view-withsize-types-125f` | View compose · Shell · **Bundle.observe = one door** | **pushed** — soft-deprecate ui.data | tip (this push) | typecheck 0 | Hold kit Dashboard; TanStack parallel | 2026-07-29 |
...
2. **Agent G / TUI:** same stack for library + apps — `Hyperlink.atom/fn` → `Bundle.observe` (soft-deprecate `ui.data` / `use*Bundle`); DashboardLayer + Shell; kit HOLD. Branch `cursor/view-withsize-types-125f`.
