...
| **G / TUI** | `cursor/file-router-prototype-125f` | [**effect app router plan**](./effect-app-router-plan.md) · [provide](./view-provide-draft.md) · [compose](./view-compose-draft.md) · Last.ts docs (`docs/last/`) | **`Last.app` shell** — Layer (+ `Last.router` / Waku `router`) → children-only `Provider`; title-live + docs RouterProvider migrated | work tip | last-app tests + last-ts tsc | API docgen for last-ts | 2026-08-05 |
