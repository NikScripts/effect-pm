...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/hyperservice-open-deps-5679` | [fleet rate limiting](../plans/fleet-rate-limiting.md) · [wire groups](../plans/wire-groups-and-identity.md) · [service shapes](../plans/service-shapes.md) | **R3b live Redis Eng’d** (work branch) — R1+R3 on tip; Redis Gate/WorkPool + Persistence/PersistedQueue live suites | `db68a4e0` | typecheck 0 / redis live 7 + gate fleet green | R2 `metrics` nest · R4 HttpApiClient+ApiMetrics · tip-sync when owner OK | 2026-07-27 |
...
1. **Agent 4:** W1–W3 + `default`/`defaults` + **R1+R3** on tip; **R3b live Redis** on work branch ([`../plans/fleet-rate-limiting.md`](../plans/fleet-rate-limiting.md)). Next after tip-sync: R2 `metrics` nest → R4 HttpApiClient Tag + absorb ApiMetrics. Incident: [`agent-04-w3-incident-2026-07-27.md`](./agent-04-w3-incident-2026-07-27.md).
