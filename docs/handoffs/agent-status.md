...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Update](../guides/update.md) · [addresses dock](./node-addresses-and-update-api.md) · [Versioned](./versioned-schema-decisions.md) | **design** — policy both-ways sugar; `Node.config`/`Node.policy` | ahead of last tip-sync | tip still `withPolicy` | Eng when go: yield("Refuse")≡yieldRefuse (+ pick same); config/policy split. Dream; S14; stream/ref; Update parked; Sync | 2026-08-13 |
