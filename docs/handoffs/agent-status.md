...
| **5** | `cursor/lifecycle-defer-start-929b` | [Lifecycle guide](../guides/lifecycle.md) · [kernel plan](../plans/lifecycle-kernel.md) · [P-locks](./lifecycle-kernel-decisions.md) · [launcher brief](./launcher-and-handoff-brief.md) | **Eng’d ahead of tip** — Tag overloads on `start`/`pause`/`resume`/`stop` (drop `*From`); P8 remote proof | ahead of `integration` | typecheck green; lifecycle local+remote OK | Next: owner `@locked` P8; tip-sync on ask; Agent G chrome | 2026-07-31 |
...
3. **Agent 5 (Launcher + handoff):** B+P11 tip-synced; **Tag overloads** on `Lifecycle.start`/`pause`/`resume`/`stop` (no `*From`); P8 remote proof Eng’d — await owner `@locked` P8. Next: tip-sync on ask / `restartSuccessor` / #35–37. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
