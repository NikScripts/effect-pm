...
| **G / TUI** | `cursor/tui-dashboard-parity-125f` | TUI ↔ web Dashboard Group parity · `View` redesign | **Eng** — web/tui `DashboardViews` skins for all families | tip-sync after push | ui-dashboard-views + ui-view + workpool-view; tsgo ui/web/tui | Next: wire Dashboards to `DashboardViews.layer` (drop WorkPool-only); **hold** kit Dashboard. Notes: [`client-adapters-design.md`](./client-adapters-design.md). | 2026-07-27 |
...
2. **Agent G / TUI:** Dashboard Group parity + View redesign (`cursor/tui-dashboard-parity-125f`) — WorkPool on existing Dashboard via View; hold kit Dashboard; next other widget families.
