...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/beta0-release-prep-125f` | beta.0 polish (Gate shorts, workPool* internals, hyperlink/`hyp`) | **ready** — Eng green; on `integration` tip | same tip as `integration` | typecheck+lint+targeted | Owner still: publish, `effect-hyperlink` keep/unpublish, docs domain; `hyp` = repo-dev (separate) | 2026-07-24 |
