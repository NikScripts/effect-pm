...
| **G / TUI** | `cursor/file-router-prototype-125f` | [**router HttpApi lock**](./router-httpapi-lock.md) · [effect app router](./effect-app-router-plan.md) | **Eng’d** mix `HttpApiGroup`/`HttpApiEndpoint` into `Router.make` / `Router.group` (URL surface) + HttpApi-faithful builder | work tip | ui-routes + router-builder green; last-ts tsc green | real `Waku.layer`; retire `Route.handle` Outlet path | 2026-08-06 |
