...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Versioned](./versioned-schema-decisions.md) · [deprecated](../guides/deprecated.md) · [Lifecycle](../guides/lifecycle.md) | **Lookup A/B dial+orch Eng'd** — follow + handoff recipe | same tip as `integration` | lookup-follow + lookup-follow-handoff | Next Eng: Launcher ensure-Lookup-first | 2026-08-04 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. **Versioned + `Hyperlink.deprecated` + `Lookup.follow` + ownership handoff recipe Eng'd**. **Next Eng:** Launcher ensure-Lookup-first; then impact / app A/B. Also gated: #37, Track D redirect. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
