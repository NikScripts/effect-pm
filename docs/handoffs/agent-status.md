...
| **G / TUI** | `cursor/file-router-prototype-125f` | [**router HttpApi lock**](./router-httpapi-lock.md) · [effect app router](./effect-app-router-plan.md) | **Eng’d RouterBuilder** — `Router.make` catalog, `RouterBuilder.group(layout, handlers)`, `Memory`/`History.layer`, `Last.provider` | work tip | router-builder + ui-router/routes green | `.from(Service)` file routes; flatten Group.asRoutes; kill Route.handle path | 2026-08-05 |
