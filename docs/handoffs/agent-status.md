...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Update](../guides/update.md) · [addresses dock](./node-addresses-and-update-api.md) · [Versioned](./versioned-schema-decisions.md) | **tip-synced** — PolicyBuilder Schema keys; defaults = Reference `defaultValue`; no `Family` | same tip as `integration` | policy + policy-builder suites green | Node.make / Host|Machine / Eng NodePolicy + Policy→LookupPolicy | 2026-08-10 |
