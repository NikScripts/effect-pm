...
| **G / TUI** | `cursor/view-withsize-types-125f` | View compose · [UI Route](./ui-routes-dream.md) | **landed** — `fromEffect` UrlBuilder typed (nested under `topLevel`) | same tip as `integration` | ui-routes + ui-router + `.test-d.ts` | F5 flake pre-existing | 2026-07-29 |
...
2. **Agent G / TUI:** Router dream machine **landed** — `Route` + `Router` only; `Route.handle` + `Router.Outlet`; Group trees via `Group.asRoutes` + `fromEffect` with **typed** UrlBuilder (including nested groups under `topLevel`) ([`ui-routes-dream.md`](./ui-routes-dream.md)). Tip-synced with `integration`.
