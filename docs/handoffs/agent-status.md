...
| **5** | `cursor/launcher-handoff-design-929b` | [launcher + handoff brief](./launcher-and-handoff-brief.md) · [rename](./rename-hyperlink-handoff.md) | **landing** — serve-site handoff (#39); WorkPool bake | tip-sync after land | typecheck 0; handoff 13/13 | `restartSuccessor` / serve `{ node }` / #35–37 deferred | 2026-07-29 |
...
3. **Agent 5 (Launcher + handoff):** Landing serve-site `handoff(from,to,ctx)` (#39); WorkPool.serve bakes `releaseEnqueueHandoff`. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
