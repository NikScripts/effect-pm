...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/tui-dashboard-parity-125f` | TUI ↔ web Dashboard Group parity | **in flight** — shipped Group Ink `Dashboard` + `Tui.open({ tree, path })`; local Fleet live-tested | (push tip) | `vitest test/resource-cli`; `example:dashboard` / `example:hyperlink -- Mail` live vs servers | Sync to `integration` after review; more leaf kinds (priority/gate/…) still web-ahead | 2026-07-24 |
