...
| **G / TUI** | `cursor/view-withsize-types-125f` | View compose · [UI Route](./ui-routes-dream.md) · [site routes](./waku-site-routes-api.md) | **full/lite** — `Router/waku` + lite `make`; GroupNav on top | tip ahead of `integration` until sync | ui-router tests | ask before sync | 2026-07-30 |
...
2. **Agent G / TUI:** Router dream — lite `Router.make` + full `ui/Router/waku`; GroupNav on top; site skins full edition ([`ui-routes-dream.md`](./ui-routes-dream.md)). Ask before syncing `integration`.
