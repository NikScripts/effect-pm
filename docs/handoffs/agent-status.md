...
### Ready / owner calls
4. **Node catalog + identity lookup** — design [`node-catalog-and-discovery.md`](./node-catalog-and-discovery.md); IPC **I1–I5**, **L1**, **X5**, **S1** LOCKED + Eng shipped (Lookup claim, ProtocolKind tags, `Resource.identity` claim→local-or-client, one-Node rule); **next:** bake **C1** (Tags + node sets) — no further Eng without lock; C2–C5 / nodeless / managers / X1 still OPEN
