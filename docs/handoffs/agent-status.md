...
| **G / TUI** | `cursor/file-router-prototype-125f` | [last-ts codesplit](../plans/last-ts-codesplit.md) · [file-router](./file-router-prototype.md) | **tip-synced** — P0–P1 `packages/last-ts` (Page, AtomReact, vite); hyperlink re-exports | same tip as `integration` | ui tsc + ui-file-router | Next: Page.Tag + createPages; then P2 Route/Router · P3 View→last-ts + `Ui` | 2026-08-02 |
...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Update + addresses](./node-addresses-and-update-api.md) · [Versioned](./versioned-schema-decisions.md) · [Lifecycle](../guides/lifecycle.md) | **tip-synced** — Update module design: fleet plan, simulate, Machine/spine α reopen | same tip as `integration` | design doc only (no Eng) | Discuss Machine vs Launcher + plan ordering; dream-redeploy provisional | 2026-08-08 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. Lookup-first + planUpdate + Dialers / sticky dual-serve Eng'd. **`restartSuccessor` rejected as SSOT** — design dock [`node-addresses-and-update-api.md`](./node-addresses-and-update-api.md): `Update` module (plan→simulate→execute, fleet order, version audit), addresses, Update/Machine node; **spine α may reopen**. Discuss before Eng. Gated: #37. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
