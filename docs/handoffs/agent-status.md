...
2. **Agent G / TUI:** [`effect-app-router-plan.md`](./effect-app-router-plan.md) — Views use plain `Layer`/`Effect` (no `View.succeed`/`gen` masks); `View.mount(Service)` only; `Document.title` static Layers; `Last.provide` → `Last.toLayer`. **Need owner:** lock plan + B1, tip-sync. Park: [`owned-string-casing-park.md`](./owned-string-casing-park.md).
