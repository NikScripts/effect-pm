...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Versioned](./versioned-schema-decisions.md) · [Lifecycle](../guides/lifecycle.md) | **Lifecycle closed** (G chrome); Versioned locked (tip id / Class / one system); `.deprecated` + update-impact proposed | same tip as `integration` | Lifecycle suites green on tip | Lock: deprecated name · impact API home; then Eng go Versioned | 2026-08-03 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. **Versioned** locked on tip (per-tip id / Class.identifier / one `schemaVersion` system) — Eng on go. Proposed (not locked): `Hyperlink.deprecated` wire-only methods; launcher/Lookup **update impact** plan. Also gated: explicit A/B, `restartSuccessor`, #36–37, Track D redirect. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
