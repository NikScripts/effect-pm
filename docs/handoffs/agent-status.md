...
| **G / TUI** | `cursor/file-router-prototype-125f` | [typed JSX / compose](./view-compose-draft.md) · [page-layout](./page-layout-design.md) · [file-router](./file-router-prototype.md) | **Design locked** — fix JSX erasure (child→parent→fn `R`); View gen Eng’d | work tip | last-ts tsc + view suites | Owner go on `last-ts/jsx` Eng; Page.Tag + tip-sync later | 2026-08-03 |
