...
| **G / TUI** | `cursor/tui-dashboard-parity-125f` | TUI ↔ web Dashboard Group parity · `View` redesign · **F5 split-dial** | **Eng** — Dashboards on `DashboardViews.layer`; F5 Eng'd; **fix** relative `Hyperlink.ws` verify (addressed multi-protocol) | `e2666a4d5` | verify-connection + ui-node-status-bundle green; hub `Hyperlink.ws("/rpc")` builds; node status up | Next: peel DetailScreen; **hold** kit Dashboard. Hard-refresh http://localhost:5177/ | 2026-07-27 |
