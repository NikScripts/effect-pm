...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [addresses + update API](./node-addresses-and-update-api.md) · [Versioned](./versioned-schema-decisions.md) · [Lifecycle](../guides/lifecycle.md) | **tip-synced** — design notes: main/additional addresses; `restartSuccessor` redesign dock | same tip as `integration` | design doc only (no Eng) | Discuss fix for update API before address Eng; dream-redeploy provisional | 2026-08-07 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. Lookup-first + planUpdate + Dialers / sticky dual-serve Eng'd. **`restartSuccessor` + dream redeploy = Eng'd but owner-rejected as SSOT** — redesign dock + address model (main + A/B, proxy lean): [`node-addresses-and-update-api.md`](./node-addresses-and-update-api.md). Discuss API fix before Eng. Gated: #37. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
