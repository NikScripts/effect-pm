...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Update](../guides/update.md) · [addresses dock](./node-addresses-and-update-api.md) · [Versioned](./versioned-schema-decisions.md) | **design** — policy≠config locked; dream β rejected | ahead of last tip-sync | forward-proxy OK as thin Shape β; tip still merged Policy bags | Eng when go: `LookupConfig`/`NodeConfig` + shrunk handler `LookupPolicy` (Yield/Pick). Dream rewrite; S14 handoff peer; stream/ref forward; Update parked; tip-sync when Sync | 2026-08-12 |
