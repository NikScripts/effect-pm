...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/hyperservice-open-deps-5679` | [wire groups & identity](../plans/wire-groups-and-identity.md) · [service shapes](../plans/service-shapes.md) | **Eng W1** — drop public `groupId` | tip-sync after push | typecheck + targeted tests | W2 demote unused family APIs next | 2026-07-27 |
...
1. **Agent 4:** Wire identity **W1 Eng** ([`wire-groups-and-identity.md`](../plans/wire-groups-and-identity.md)) — drop public `groupId`, wire=`.key` / `wireKeyOf`. Service shapes: `Tag`/`value`/`promise`/`pure` Eng’d; Creating polish + handle adornments paused. Next: W2 demote unused family APIs.
