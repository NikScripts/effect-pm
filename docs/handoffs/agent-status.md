...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/hyperservice-open-deps-5679` | [wire groups & identity](../plans/wire-groups-and-identity.md) · [service shapes](../plans/service-shapes.md) · [W3 incident](./agent-04-w3-incident-2026-07-27.md) | **W1–W3 Eng’d** — shared `Tag(wireKey, spec)` (no `*Family*`); ApiMetrics migrate + `.handle` rename **paused** | `382c04a7` | typecheck + resource/shared-tag tests | **do not push `integration` without owner OK**; wait on metrics / handle name | 2026-07-27 |
