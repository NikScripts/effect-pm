...
| **G / TUI** | `cursor/view-withsize-types-125f` | View compose · [UI Route](./ui-routes-dream.md) · [owned-string park](./owned-string-casing-park.md) | **tip-synced** — Router/Target `_tag` cutover + owned-string Eng | same tip as `integration` | ui-router / typecheck (pre-existing example errors) | park: value/ref, WorkPool modes, Href brand | 2026-07-31 |
...
2. **Agent G / TUI:** Router `Service._tag` + `Route.TargetValue` tagged sum + PathToken `_tag`; Target.view locked A; parked wire-heavy (`value`/`ref`, WorkPool modes, Href) in [`owned-string-casing-park.md`](./owned-string-casing-park.md) — **tip-synced**.
