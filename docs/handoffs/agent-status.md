...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Update](../guides/update.md) · [addresses dock](./node-addresses-and-update-api.md) · [Versioned](./versioned-schema-decisions.md) | **tip-synced** — Address/`NodePolicy` design (`as`, not Role; client `Policy` separate) | same tip as `integration` | Update suite green; Address/NodePolicy design only | Node.make / Host|Machine / Eng NodePolicy | 2026-08-10 |
