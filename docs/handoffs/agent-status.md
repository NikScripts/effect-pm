...
| **G / TUI** | `cursor/view-withsize-types-125f` | View compose · [UI Route](./ui-routes-dream.md) · [owned-string park](./owned-string-casing-park.md) | **on branch** — `_tag` cutover + docs/examples/tests polish (ask before `integration` sync) | ahead of last ask | ui-router + group-nav example | park: value/ref, WorkPool modes, Href brand | 2026-07-31 |
...
2. **Agent G / TUI:** Router/Target `_tag` cutover + routing guide in nav + GroupNav example/tests; **do not sync `integration` without asking**. Park: [`owned-string-casing-park.md`](./owned-string-casing-park.md).
