...
| **K** | `cursor/agent-k-page-route-6d0e` | [**Agent K handoff**](./agent-k-page-route-handoff.md) · [page/route lock](./page-route-make-lock.md) · [rsc-router](../last/rsc-router.md) | **ahead of `integration`** — deleted never-approved `pageConfig`; engine `getConfig` on param/dynamic files | b53696104 | vitest page/route; curl `/` `/guides/routing` `/docs/intro/rest` 200 | owner: PR land + `version` | 2026-08-08 |
...
2. **Agent K:** last-ts + file-router on `cursor/agent-k-page-route-6d0e`. **`pageConfig` deleted** (never-approved bridge). Static via `Page.static` / `Route.staticFromEffect` / engine `getConfig` on param files only. Handoff: [`agent-k-page-route-handoff.md`](./agent-k-page-route-handoff.md). Park: [`owned-string-casing-park.md`](./owned-string-casing-park.md).
