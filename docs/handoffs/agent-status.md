...
| **G / TUI** | `cursor/view-withsize-types-125f` | View compose · **DashboardLayer + Shell** · Bundle.observe | **pushed** — shell extract | tip (this push) | typecheck 0; atom+bundle+view-data 10/10 | Hold kit Dashboard; TanStack parallel | 2026-07-29 |
...
2. **Agent G / TUI:** View compose + detail peel + **`DashboardLayer.forCompose` + platform `DashboardShell`** (Dashboard = thin wiring; kit HOLD) + Bundle.observe dogfood on `cursor/view-withsize-types-125f`. Guides: view-tag-types · hyperlink-atom · bundles.
