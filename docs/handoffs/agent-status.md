...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/hyperservice-open-deps-5679` | [fleet rate limiting](../plans/fleet-rate-limiting.md) · [wire groups](../plans/wire-groups-and-identity.md) · [service shapes](../plans/service-shapes.md) | **Eng** — R4b ApiMetrics delete + dashboard nest parity | same tip as `integration` after push | typecheck + api-metrics suites | tip-sync after green | 2026-07-28 |
...
| **5** | `cursor/launcher-handoff-design-929b` | [launcher + handoff brief](./launcher-and-handoff-brief.md) · [rename](./rename-hyperlink-handoff.md) | **URLs + vocab** — on `integration` tip | same tip as `integration` | typecheck + docs:manifest | Owner: lock C #27–36 | 2026-07-28 |
...
1. **Agent 4:** W1–W3 + `default`/`defaults` + R1–R4+adaptive + A1–A3 tip-synced; **`cell` parked/rejected**; **R5 rejected**; **R4b Eng** — delete sibling `ApiMetrics`, dashboard/TUI surface limiter nest. Incident: [`agent-04-w3-incident-2026-07-27.md`](./agent-04-w3-incident-2026-07-27.md).
