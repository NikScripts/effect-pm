...
| **Launcher** | `cursor/launcher-handoff-design-929b` | [launcher + handoff brief](./launcher-and-handoff-brief.md) | **beta audit done** — A+B Eng'd + hardened | tip-synced | launcher+assume 10/10; typecheck green | Next: Track C bake (owner) | 2026-07-27 |
| **Docs site** | `cursor/docs-site-edge-cache-dbdc` (ε) | [origin down](./docs-site-origin-down.md) · edge cache · SEO · Lighthouse | **tip-synced** — API SSG restored; #418 fixed | same tip as `integration` | WorkPool/module pages 200; 1873 SSG files; LH install 100s | 1Password CLI deferred; **no landing UI without owner OK** | 2026-07-27 |
...
3. **This agent (Launcher):** Track A+B Eng'd + **beta hardened** (CSPRNG tokens, `HandleNotReady`, logs, guide). Ready for Track C bake. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md). Guide: [`docs/guides/launcher.md`](../guides/launcher.md).
4. **Docs site:** tip-synced + redeployed. React **#418** fixed; truncated API SSG fixed (module/package pages 200 again). Lighthouse install 100s. Landing UI unchanged. 1Password CLI deferred.
