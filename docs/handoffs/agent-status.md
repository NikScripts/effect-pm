...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Update + addresses](./node-addresses-and-update-api.md) · [Versioned](./versioned-schema-decisions.md) · [Lifecycle](../guides/lifecycle.md) | **tip-synced** — corrected: tip has `Node.Service` (G Tag→Service); `Node.make` still design | same tip as `integration` | design doc only (no Eng) | G: no reply yet on Page mint vs `Node.make`; Host/Machine open | 2026-08-08 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. Lookup-first + planUpdate + Dialers / sticky dual-serve Eng'd. **`restartSuccessor` rejected as SSOT** — design dock [`node-addresses-and-update-api.md`](./node-addresses-and-update-api.md). **Sync corrected:** tip has **`Node.Service`** (G `a3831b5f` Tag→Service). Lean still `Node.make` (HttpApi description) + locality under Node; keep Launcher. **G:** no written answer yet on Page mint vs make. Gated: #37.
