...
| **G / TUI** | `cursor/file-router-prototype-125f` | [**router HttpApi lock**](./router-httpapi-lock.md) · [effect app router](./effect-app-router-plan.md) | **Eng’d** HttpApi-faithful builder (`group.key`, `Handlers`/`ValidateReturn`, `Group.Service`/`ToService`, `Registry`) + `from(Service)` / flat `asRoutes` | work tip | router-builder + ui-router green; typecheck = tip | real `Waku.layer`; retire `Route.handle` Outlet path | 2026-08-06 |
