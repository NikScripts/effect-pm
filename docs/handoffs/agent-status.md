...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Versioned](./versioned-schema-decisions.md) · [deprecated](../guides/deprecated.md) · [Lifecycle](../guides/lifecycle.md) | **tip-synced** — dream redeploy file-swap e2e (v1→v2 + sticky + WorkPool handoff) | same tip as `integration` | example + `test/launcher-dream-redeploy.test.ts` green | Explicit Redirect SDK deferred; stream resume optional | 2026-08-07 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. **Lookup-first + already-up + `Lookup.planUpdate` + `Launcher.restartSuccessor` + Dialers / sticky dual-serve + dream file-swap redeploy Eng'd**. Optional: stream resume tokens / seam dedupe. Gated: #37. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
