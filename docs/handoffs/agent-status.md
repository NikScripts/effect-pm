...
2. **Agent G / TUI:** [`effect-app-router-plan.md`](./effect-app-router-plan.md) — `Document.title` static Layers; `Last.provide` in page; body-only return; **bag `View.succeed({…})` removed**. **Need owner:** lock plan + B1, tip-sync. Park: [`owned-string-casing-park.md`](./owned-string-casing-park.md).
