...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/hyperservice-open-deps-5679` | [examples IA](./examples-ia-reorg.md) · [gap proposals](./examples-gap-proposals.md) · [E5 apps](./examples-apps-e5-plan.md) | **Plan** — examples gap queue for owner walk (Idea 1 WorkPool) | same tip as `integration` (pending) | tip-synced + proposal doc | Owner: pick Idea 1 subset / confirm order | 2026-07-29 |
<<<<<<< HEAD
...
1. **Agent 4:** Examples IA Eng’d. **Gap proposals** for missing forms — walk Idea 1 (WorkPool) then next. Handoff: [`examples-gap-proposals.md`](./examples-gap-proposals.md). E5 still owner-gated.
<<<<<<< HEAD
