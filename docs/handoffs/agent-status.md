...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Versioned](./versioned-schema-decisions.md) · [deprecated](../guides/deprecated.md) · [Lifecycle](../guides/lifecycle.md) | **planUpdate Eng'd** — update-impact dry-run | same tip as `integration` | lookup-plan-update + launcher suites | Next Eng: restartSuccessor / explicit app A/B | 2026-08-05 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. **Lookup-first + already-up + `Lookup.planUpdate` Eng'd**. **Next Eng:** `restartSuccessor` / explicit app A/B. Also gated: #37, Track D redirect. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
