...
| **5** | `cursor/lifecycle-defer-start-929b` | [Lifecycle guide](../guides/lifecycle.md) · [kernel plan](../plans/lifecycle-kernel.md) · [P-locks](./lifecycle-kernel-decisions.md) · [launcher brief](./launcher-and-handoff-brief.md) | **tip-synced** — L1/L2 Eng’d (`stop`, `lifecycle._tag`, `deferStart`) | same tip as `integration` | typecheck + lifecycle/work-pool + example | L3+ open (caps / LifecycleView / handoff gate) | 2026-07-31 |
...
3. **Agent 5 (Launcher + handoff):** Lifecycle L1/L2 Eng’d (`stop`, drop `phase`, `deferStart`) + docs/example — **tip-synced**. Next: L3+ / `restartSuccessor` / #35–37. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
