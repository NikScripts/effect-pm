...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Versioned](./versioned-schema-decisions.md) · [Lifecycle](../guides/lifecycle.md) | **Versioned Eng'd** — module + WorkPool/status/durable seams; deprecated/impact deferred | same tip as `integration` | versioned.test + descriptor tests | Next after tip-sync: deprecated dual (queued) | 2026-08-03 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. **Versioned Eng'd** (`hyperlink-ts/Versioned`, status/durable/WorkPool wire). **Next (queued):** `Hyperlink.deprecated` dual/pipe; then Lookup/Node **update impact** / explicit A/B / `restartSuccessor`. Also gated: #36–37, Track D redirect. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
