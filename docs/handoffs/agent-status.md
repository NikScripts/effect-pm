...
| **Docs site** | `cursor/docs-site-edge-cache-dbdc` (ε) | [dev host](./docs-site-dev-host.md) · edge cache · SEO | **live** — apex coming-soon / docs on `dev` | same tip as `integration` (`c33f1e10a`) | smoke 38/38 on `dev`; CF Single Redirects gate brand host | `@pm` alias; optional middleware fix (static bypass) | 2026-07-28 |
...
4. **Docs site:** Live — `hyperlink.cool` coming-soon; docs at `https://dev.hyperlink.cool`. Deploy `c33f1e10a`, smoke 38/38, CF Single Redirects (origin middleware bypassed by Waku static). `@pm` alias still open.
