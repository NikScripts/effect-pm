...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Update](../guides/update.md) · [addresses dock](./node-addresses-and-update-api.md) · [Versioned](./versioned-schema-decisions.md) | **tip-synced** — Update perfection (target-scoped blockers, shared gate, contracts) | same tip as `integration` | `test/update.test.ts` (15) + dream-redeploy + plan-update | Node.make / Host|Machine / proxy still design | 2026-08-08 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent K** (was G). **`Update` perfected** — target-scoped status blockers, shared sim/exec gate, fail-closed contracts, planned-impact execute ([`update.md`](../guides/update.md)). Address/`Node.make`/locality still design dock. Gated: #37.
