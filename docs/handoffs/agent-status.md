...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/hyperservice-open-deps-5679` | [fleet rate limiting](../plans/fleet-rate-limiting.md) · [wire groups](../plans/wire-groups-and-identity.md) · [service shapes](../plans/service-shapes.md) | **tip-synced** — R1–R4+adaptive on `integration` | same tip as `integration` | typecheck 0 / api-metrics+http-api green | next: `cell` bake or R5 optional | 2026-07-27 |
...
| **Docs site** | `cursor/docs-site-edge-cache-dbdc` (ε) | [origin down](./docs-site-origin-down.md) · edge cache · SEO | **tip-synced** — SEO + processSlot live; landing UI reverted (owner gate) | same tip as `integration` | `/healthz` ok; Coming soon restored; `yield*` in meta; favicon/og/canonical; CF HIT | 1Password CLI backup deferred; **no landing UI without owner OK** | 2026-07-27 |
=======
| **Launcher** | `cursor/launcher-handoff-design-929b` | [launcher + handoff brief](./launcher-and-handoff-brief.md) | **bake** — product 1 + Track A API | branch tip | docs only | Spawn input locked (`node`+`ChildProcess`+`ready?`). Open: custody vs flat, assume wire, timeouts | 2026-07-27 |
| **Docs site** | `cursor/docs-site-edge-cache-dbdc` (ε) | [origin down](./docs-site-origin-down.md) · edge cache · SEO/landing | **tip-synced** — SEO/landing + processSlot Shiki latch live | same tip as `integration` | `/healthz` ok; CTAs; `yield*` in meta; favicon/og/canonical; CF HIT; build 0 Shiki warnings | 1Password CLI backup deferred (desktop Integrate with CLI) | 2026-07-27 |
>>>>>>> origin/integration
