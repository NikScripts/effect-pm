...
|-------|--------|---------|-------|---------|--------------|-----------------|---------------|
| **4** | `cursor/hyperservice-open-deps-5679` | [fleet rate limiting](../plans/fleet-rate-limiting.md) · [wire groups](../plans/wire-groups-and-identity.md) · [service shapes](../plans/service-shapes.md) | **Eng** — R1 Gate `rateLimit` (presence Soft memory); W1–W3 + `default`/`defaults` on `integration` | `ade870a4` | typecheck 0 / gate.test 27 | R2 observe nest · R3 Redis recipe · R4 HttpApiClient+ApiMetrics | 2026-07-27 |
...
1. **Agent 4:** W1–W3 + `default`/`defaults` **on `integration`**. **R1 Eng’d** on work branch — Gate `rateLimit` + presence-driven Soft memory ([`../plans/fleet-rate-limiting.md`](../plans/fleet-rate-limiting.md)). Next: R2 observe nest → R3 Redis fleet recipe → R4 HttpApiClient Tag + absorb ApiMetrics. Ask before landing R1 on `integration`. Incident: [`agent-04-w3-incident-2026-07-27.md`](./agent-04-w3-incident-2026-07-27.md).
