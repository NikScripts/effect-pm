...
| **G / TUI** | `cursor/file-router-prototype-125f` | [view compose / mount](./view-compose-draft.md) · [**upward provide**](./view-provide-draft.md) · [page-layout](./page-layout-design.md) · [file-router](./file-router-prototype.md) | **Eng’d down** (Unresolved + bag + mount); **upward Provides/`Last.provide` requirements written** — not Eng’d | work tip | last-ts tsc + view-jsx + Twoslash demo | V0 provide Eng; Page.Tag; tip-sync later | 2026-08-04 |
...
2. **Agent G / TUI:** last-ts P0–P3 + View handle/`fromEffect`/`gen` + **bag compose + `View.mount`** (open-`R` not JSX-legal) on work tip. **Upward** Requires/Provides/`Last.provide` requirements in [`view-provide-draft.md`](./view-provide-draft.md) — Eng on go. **Need owner:** V0 provide Eng, `Page.Tag` go, docs-site createPages cutover, tip-sync. P4 shims remain. Park: [`owned-string-casing-park.md`](./owned-string-casing-park.md).
