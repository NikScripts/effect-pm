...
| **5** | `cursor/lifecycle-defer-start-929b` | [Lifecycle guide](../guides/lifecycle.md) · [kernel plan](../plans/lifecycle-kernel.md) · [P-locks](./lifecycle-kernel-decisions.md) · [launcher brief](./launcher-and-handoff-brief.md) | **tip-synced** — P10 Gate Lifecycle; `GateStopped` on wire `run` (`Schema.TaggedErrorClass` + `withGateStopped`); P12 Rejected | same tip as `integration` | typecheck 0 `error TS`; gate-lifecycle + gate-handle.test-d | Residual: `RateLimiterError` still erased from wire; Agent G chrome; launcher deferred | 2026-08-01 |
