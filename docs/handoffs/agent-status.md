...
| **Docs site** | `cursor/docs-site-edge-cache-dbdc` (ε) | [origin down](./docs-site-origin-down.md) · edge cache · SEO · Lighthouse | **Eng** — `DurableQueueStore` → `DurableWorkPoolStore` | pending tip-sync | rename + changeset; SQLite table `durable_work_pool` | tip-sync OK?; then Getting Started refine; `@pm` alias | 2026-07-28 |
...
4. **Docs site:** Eng-renaming `DurableQueueStore` → `DurableWorkPoolStore` (port, error, SQLite backend, table `durable_work_pool`). Next: tip-sync (needs OK) + Getting Started refine + redeploy. `@pm` alias still open.
