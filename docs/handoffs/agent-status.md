...
| **K** | `cursor/file-router-prototype-125f` | [**Agent K handoff**](./agent-k-page-route-handoff.md) · [page/route lock](./page-route-make-lock.md) · [rsc-router](../last/rsc-router.md) | **tip-synced** with `integration` — fileRootFromPages + rest page Eng’d | same tip as `integration` | vitest page/route green; live `:5220` smoke unfinished | owner: PR + `version` | 2026-08-08 |
...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Update](../guides/update.md) · [addresses dock](./node-addresses-and-update-api.md) §3.3.3 | **design pack** — D1–D20 agent-decided (owner override OK) | ahead of last tip-sync | dream sketch in dock; tip not Eng’d | Owner: change any D# · Eng order D20: S14 handoff → config/policy → stream/ref → Update β → dream rewrite | 2026-08-15 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent K** (was G). **`Update` 2nd-pass refine** — array-derived blockers, shape re-validation, `PlanError`/`ExecuteError`, `UpdateReport`, `update.*` spans, `Node.shutdown` Advice leave fix ([`update.md`](../guides/update.md)). Address/`Node.make`/locality still design dock. Gated: #37.
