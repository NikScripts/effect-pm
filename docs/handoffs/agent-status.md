...
| **5** | `cursor/lifecycle-defer-start-929b` | [launcher brief](./launcher-and-handoff-brief.md) · [Versioned](./versioned-schema-decisions.md) · [deprecated](../guides/deprecated.md) · [Lifecycle](../guides/lifecycle.md) | **restartSuccessor live e2e** — plan → OS up B → shutdown A (askIncumbent dial-replace) | same tip as `integration` | restart-successor live + plan-update + launcher suites | Dual-serve / redirect / live clientsAtRisk still deferred | 2026-08-07 |
...
3. **Agent 5 (Launcher + handoff):** **Lifecycle L0–L7 closed**. Chrome → **Agent G**. **Lookup-first + already-up + `Lookup.planUpdate` + `Launcher.restartSuccessor` Eng'd** (ambient Layers + live OS A→B e2e). **Next:** dual-serve / redirect / live `clientsAtRisk` (owner-gated). Also gated: #37, Track D redirect. Brief: [`launcher-and-handoff-brief.md`](./launcher-and-handoff-brief.md).
