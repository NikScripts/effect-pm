...
**Order:** After Agent A fills `docs/site/standards/`.  
...
Scan `src/`, `test/`, `examples/`, `docs/` against `docs/site/standards/manifest.json` rules. **Stricter than owner** — flag `should` and `must`. Propose fixes; owner rejects overreach.

Publish `docs/site/audits/YYYY-MM-DD-report.html` (served like standards). Open draft PR with **top 5** clearest fixes only.
...
3. Write HTML audit report under `docs/site/audits/`
