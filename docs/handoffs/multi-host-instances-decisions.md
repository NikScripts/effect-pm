...
2. **The `Host` carries its url** (`Resource.Host("id", { url })`), and the fleet is piped on with
   `.pipe(Resource.multiHost([hosts]))` — hostless, no primary; every instance is an equal peer. (It's
