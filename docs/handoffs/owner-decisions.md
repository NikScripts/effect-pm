...
- **Chose (bake 2026-07-27 — rateLimit keying):** v1 **whole-client only** — one policy; `rateLimit.key` optional (default Tag id). **Discovery must not live under the renameable nest path** (widgets can’t find `gh.clientMetrics.*` by guessing). Stable surface on the Tag/handle metadata (e.g. statics / fixed non-renamed fields): resolved **bucket key** + **metrics nest key** (`metricsKey`). Nest holds live data only (`usage`, `remaining`, …). Per-route keys later.
