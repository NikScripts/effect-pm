...
- **Chose / Eng’d:** `Hyperlink.default(value)` (literal or sync fn; Promise-returning fn type-errors) + `Hyperlink.defaults({…})` pipe (bag on Tag via `DefaultsOf`). **Follow-up Eng’d (construction adornments A1–A2):** `defaults` remaps `Svc → Svc & Bag` via internal `remapTagService` (`as unknown as`, guarded by `test/defaults-handle.test-d.ts`); `WithDefaults` kept as escape/migration. Spec∩bag → `DuplicateDefaultKey`. Layer/serve accept `ImplWithDefaultOverrides`. `Hyperlink.pure` / `PureMethod` removed (major).
- **Rejected:** Keeping `pure` as an alias or shim; two-step Prototype mint as the product API.
- **Still open:** live `cell` (park lean); optional A3 `{ defaults }` factory sugar.
