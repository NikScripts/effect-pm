...
- **Chose (bake 2026-07-27 — rateLimit keying):** v1 **whole-client only** — one policy; `rateLimit.key` optional (default Tag id). **Resolved key is stored on the handle** (under the `metrics` nest, e.g. `metrics.key` or alongside limiter fields) so widgets / dashboard config can bind the right effects. Per-route keys later.
