...
Status: LOCKED, building autonomously on `store-storage-refactor`. NO merge to integration in this run
(that's a separate, coordinated step once the live agent branches — process +4, run +3 — are ready).
Each phase: no `as`/`any` beyond the existing tree-walk/rebuild idioms, no lazy imports, one field per line;
both tsconfig projects + Effect LSP + full suite green before it commits+pushes.
