...
| AI.8 | Wire field | **LOCKED** — advertise carries advertiser preference (`callSite`∋`nodeStamp`, may still be `"inherit"` / omit); **Lookup server** finishes resolve with its node stamp (so `layerOptions` stays `livenessReplace` unless a stamped `Node.Lookup` is served) |
