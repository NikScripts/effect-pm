...
**Views — Effect-native:** Mint with `View.make`; build layers with `Layer.succeed` / `Layer.effect` + `Effect.gen`. Edge: `View.stamp(Last.provide(Service))` (compose deps on `static layer`). Always `yield*` a Service for the component.
