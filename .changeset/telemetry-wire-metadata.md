...
- `Telemetry.Type.Codec<T>` and `Telemetry.Type.CodecTag<T, Tag>` derive public projection unions from a telemetry codec.
...
- `RunResourceStore` derives public fact/state-change types from `RunResourceCodec`.
