...
- `.pipe(Resource.multiHost([NwslHost, EbwslHost, WnbaHost]))` — pipe the fleet (host set) onto the tag; hostless, every instance an equal peer. `Resource.fleet(method)` tags a combined field (served, but excluded from `peers`).
